...
With it, I was able to show only the relevant books based on the current window, using the scrollbar.
If this would not have existed, te standard pagination approach would be been useful.
I am using material-ui for a design skeleton. Obviously with proper designers, we can use other tools as well.
Using css files and material-ui makeStyles are present. I am not quite sure which is more efficient, although using css files is a bit cleaner, especially because they are just 1 small step from scss files or styles components.

The warnings in the dev console are caused by the external react library of react-virtualized.
There are 3 options that we can consider:
1) contact the owner of the repo and ask for an update
2) fork the repo and make the changes ourselves
3) find another component

While the 1 million books app is challenging, I don't think that this is a valid usecase. There should be no case when the backend would send this many data in 1 call.
At least pagination should be implemented on the backend side.
A proposal for this is using graphql for example.
